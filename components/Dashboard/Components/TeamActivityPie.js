  ...
  useEffect(() => {
    const chartData = prUsers.map(user => {
      const prItem = pr.find(
        prs => prs.items && prs.items.length > 0 && prs.items[0].user?.login === user
      )
        ...
        <div className="flex items-center space-x-10">
          <div className="z-10 flex items-center">
            ...
            <div>
              ...
              >
                This interactive chart displays the number of Pull Requests (PRs) and contributions
                made by each contributor. Clicking on a contributor&apos;s name allows you to filter
                and compare their individual data.
