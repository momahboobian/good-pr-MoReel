...
export default function TaskActivity({ issuesClosed, issuesOpen, repo }) {
  const issues = [...issuesOpen, ...issuesClosed].sort(
    (a, b) =>
      b.items.map((el) => el.created_at) - a.items.map((el) => el.created_at)
  );

  const tasks = [];
  issues.map((el) => el.items.map((issue) => tasks.push(issue)));
                ...
                <tr key={idx}>
                  <td className="py-4 min-w-[170px] whitespace-nowrap">
                    <span className="flex flex-row items-center">
                      <Image
                        src={el.items.assignees[0].avatar_url}
                        width={40}
                        height={40}
                        alt={el.items.assignees[0].login}
